...
> **Last Reconciled**: 2026-09-03 09:53 HKT (UTC+8) — §8 量測值重數並補上量測方法；see §7 for drift found and closed
...
> **量測方法**（2026-09-03 重數，記錄下來讓日後的數字可以比對而非重新發明）：
> inline style = `grep -o 'style="' app.js | wc -l`；class = 同法數 `class="`；
> 類別選擇器 = `grep -cE '^\.[a-zA-Z][a-zA-Z0-9_-]*[^{]*\{' index.css`（只數規則開頭，不含續行）；
> 淺色覆寫 = `grep -c '\[data-theme="light"\]' index.css`；權杖引用 = `grep -o 'var(--' app.js | wc -l`。

**Styles live mostly outside the stylesheet.** `app.js` carries **801** inline `style="…"` attributes against **739** `class` attributes, while `index.css` defines **416** class-rule openings. (Milestones 7–9 moved in the right direction on the first two — new components were written with classes rather than inline styles — but the ratio is still roughly one inline style per class.) **187** of those inline styles exceed 100 characters (longest 524). Most visual decisions are therefore made inside template literals rather than in a stylesheet.

Two consequences follow directly:

1. **No scale can hold.** Across `app.js` and `index.css` there are **447** `font-size` declarations using **71 distinct sizes** (70 in `rem`, 1 in `px`); **190 of them sit between 0.70rem and 0.85rem**, a handful of steps inside a 2.4px range, which cannot express hierarchy. Spacing uses **20** distinct px values (including 1, 2, 3, 5, 7px), border-radius **11**, and there are **6** ad-hoc breakpoints (420/500/650/768/900/950px).

   > The distinct-size count rose from the 45 recorded on 2026-08-29 to 71, while the crowded 0.70–0.85rem band *fell* from 219 to 190. Both moved for the same reason: Milestones 7–9 added their components as CSS classes with their own sizes rather than as inline styles inside the existing band. That is the right direction for maintainability and the wrong one for the scale — it confirms the section's own point that a design system, not another milestone's good intentions, is what fixes this. Counted with `grep -oE 'font-size:\s*[0-9.]+(rem|px)'` over both files; spacing and radius counts re-run unchanged.
2. **The light theme cannot fully apply.** Inline styles outrank any selector, and **155** colour literals are hardcoded inside them (most frequently `rgba(255,255,255,0.02)` and `rgba(255,255,255,0.05)`, 11 times each — white surface tints that create depth on a dark ground and vanish on a light one). `index.css` carries only **44** `[data-theme="light"]` override rules against 416 class-rule openings — 34 originally, +2 in Milestone 7 (`#radar-recommendation-panel`, whose hardcoded `rgba(10,15,30,0.85) !important` put dark text on a dark panel), +4 in Milestone 8 (draft-status line, degraded banner, evaluating overlay), +4 in Milestone 9 (write-failure card, boot/view failure cards, usage panel). Each increment follows the roadmap's rule that presentation work rides along with whichever milestone touches a screen; none attempted a whole view. **Milestone 9 deliberately removed one such override** — the daily-cap input — after measuring that the neighbouring Gemini key field is dark-on-light in *both* themes, so styling one input correctly would make it inconsistent with the field beside it. The interview room remains largely illegible in the light theme — measured during the M8 peer review, the CSS tokens *do* switch correctly (`--text-muted` → `rgb(100,116,139)`), and the fault is the room's hardcoded panel backgrounds. Inline styles do also use design tokens — **707** `var(--…)` references — so the escape hatch is the exception, not the rule.
