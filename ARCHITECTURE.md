...
> **Last Reconciled**: 2026-08-31 13:28 HKT (UTC+8) — Milestone 8 peer review; see §7 for drift found and closed
...
⚠️ **What the code actually does is broader than that intent.** The guard is `if (err && err.code) throw err`, and every `DOMException` carries a truthy numeric `.code` — so ordinary storage errors (`QuotaExceededError`, `NotFoundError`, `AbortError`) are re-thrown too, where they previously returned a fallback value and let the caller continue. This changes the failure behaviour of `persistCompletedSession()`, which has no `try`, and costs the counselor an interview. Recorded as **D32** in §7; the decision stands, the predicate is wrong.
...
| D36 | `showEvaluatingOverlay()` sets `mount.style.position = "relative"` and `hideEvaluatingOverlay()` never restores it, leaving the inline style permanently on `#content-view-mount`. |
| D37 | `renderVaultDegradedBanner()` has no de-duplication, and `switchView()` has no `case "roleplay"`/`"icf_board"` and no `default`, so those views are not re-rendered — in degraded mode each locale switch inside the interview room appends another banner. Inferred from code plus a measured no-re-render; the accumulation itself was not run in degraded mode. |
| D38 | Dead CSS: `.notes-save-indicator` (`index.css:3027`) and its responsive rule (`:3831`) have no remaining users. M8 deleted `@keyframes pulse-amber-dot` but missed these two. |
